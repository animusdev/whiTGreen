...
/world/Reboot(var/reason)
	webhook_send_roundstatus("reboot")
...
/world/Error(exception/E, datum/e_src)
	if(!istype(E)) //Something threw an unusual exception
		log_runtime("Uncaught exception: [E]")
		return ..()
	...
	if(copytext(E.name,1,32) == "Maximum recursion level reached")
		...
		log_runtime("The bug with recursion runtimes has been fixed. Please remove the snowflake check from world/Error in [__FILE__]:[__LINE__]")
	...
	if(cooldown > configured_error_cooldown * configured_error_limit)
		...
		spawn(0)
			...
			if(skipcount > 0)
				log_runtime("Skipped [skipcount] runtimes in [E.file],[E.line].")
	...
	if(usrinfo) //If this info isn't null, it hasn't been added yet
		desclines.Add(usrinfo)
	if(silencing)
		desclines += "  (This error will now be silenced for [configured_error_silence_time/10] secs)"

	var/runtime_message = "Runtime in [E.file],[E.line]: [E]"
	for(var/line in desclines)
		runtime_message+="\n[line]"
	log_runtime(runtime_message)
