...
/datum/subsystem/ticker/New()
	...
	login_music  =  pickweight(list('sound/music/aerobic.wav'))
...
/datum/subsystem/ticker/fire()
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			world  <<  "<B><FONT  color='blue'>Добро  пожаловать  нахуй!</FONT></B>"
		...
		if(GAME_STATE_PLAYING)
			...
			if(!mode.explosion_in_progress  &&  mode.check_finished()  ||  force_ending)
				...
				spawn(50)
					...
					if(delay_end)
						world  <<  "\blue  <B>Администратор  отложил  хуй в сторону.</B>"
					else
...
/datum/subsystem/ticker/proc/setup()
	...
	if(hide_mode)
		...
		world  <<  "<B>Текущий  игровой  режим  -  Паутина!</B>"
		world  <<  "<B>Возможные  режимы:</B>  Шепард сжег родную хату, Набег даунов, Нюфани все скатили, Серба в лиды"
	else
		mode.announce()
...
proc/equip_fest_stuff(H  as  mob){H:equip_to_slot_or_del(new /obj/item/clothing/head/kitty(H),slot_head)}
...
/datum/subsystem/ticker/proc/declare_completion()
	...
	world  <<  "<BR><BR><BR><FONT  size=3><B>Раунд  кончен.</B></FONT>"
