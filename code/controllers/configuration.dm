...
/datum/configuration/proc/load(filename) //the type can also be game_options, in which case it uses a different switch. not making it separate to not copypaste code - Urist
	...
	for(var/t in Lines)
		...
		switch(name)
			...
			if("alert_delta")
				config.alert_desc_delta			= "Запущен механизм самоуничтожени&#255; станции. Персонал об&#255;зан подчин&#255;тьс&#255; всем инструкци&#255;м и распор&#255;жени&#255;м со стороны руководительского состава. Любое нарушение подобного приказа может каратьс&#255; смертью. Это не учеба&#255; тревога."
			if("sec_start_brig")
