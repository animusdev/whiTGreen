...
/datum/symptom/weight_gain/Activate(var/datum/disease/advance/A)
   ...
   if(prob(SYMPTOM_ACTIVATION_PROB))
      ...
      switch(A.stage)
         if(1, 2, 3, 4)
            M << "<span class='notice'>§ [pick("¬ы чув«ƒвуеƒе, чƒо ра«Ѕолнели.", "¬ы Ѕоƒол«ƒели.")]</span>"
         else
...
/datum/symptom/weight_loss/Activate(var/datum/disease/advance/A)
   ...
   if(prob(SYMPTOM_ACTIVATION_PROB))
      ...
      switch(A.stage)
         if(1, 2, 3, 4)
            M << "<span class='notice'>§ [pick("¬ы чув«ƒвуеƒе  олод.", "¬ы очень  оƒиƒе е«ƒь.")]</span>"
         else
            M << "<span class='notice'>§ ¬а∆ живоƒ урчиƒ.</span>"
