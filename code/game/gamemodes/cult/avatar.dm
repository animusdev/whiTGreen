var/avatarcreated=0      //Ѕыл ли уже «оздан јваƒар?
var/mob/living/simple_animal/avatar/demon      // лавна€ Ѕеременна€, на коƒорой о«нован аваƒар, е о абилки, и в«Є-в«Є-в«Є.

/proc/messagevictimcheck(mob/living/P as mob)      //Ѕроверка на цель « «ообщением
...
/proc/victimcheck(mob/living/M as mob)      //Ѕроверка на цель без «ообщений
...
/mob/living/simple_animal/avatar
   ...
   see_in_dark = 8
   environment_smash = 3
   mob_size = MOB_SIZE_LARGE      //„ƒобы в ∆кафчике не заварили
   a_intent = "harm"
   ...
   minbodytemp = 0
   maxbodytemp = INFINITY      //Ѕофи  на аƒмо«. ¬озможно «ƒоиƒ убраƒь, ибо Ѕочему бы не даƒь возможно«ƒь е о зажариƒь/ко«мироваƒь?
   atmos_requirements = list("min_oxy" = 0, "max_oxy" = 0, "min_tox" = 0, "max_tox" = 0, "min_co2" = 0, "max_co2" = 0, "min_n2" = 0, "max_n2" = 0)
   faction = list("faithless","cult")      //„ƒобы е о но ами не заЅинали е о же «об«ƒвенные демоны
   var/stepsound=0      //«колько ∆а ов до звука?
   var/enraged=0      //¬ €ро«ƒи?
   var/victim=null      //∆ерƒва Ѕод воздей«ƒвием Doom. Ќужна дл€ ƒо о чƒобы Ѕрокл€ƒье можно было «кинуƒь ударив библией, а ƒак же оЅределиƒь е«ƒь ли она вообще в мире
   var/hunger=200      //Need...moar...souls
   var/summoned_during_cult=0      //≈ о Ѕризвали во врем€ кульƒа?
   var/attack_cooldown=2      // улдаун аƒаки
...
////////////////////////  Ћј¬Ќџ≈ Ѕ–ќ » ////////////////////////
   ...
   New()
      ..()
      if (avatarcreated || demon)      //≈«ƒь ƒолько один јваƒар.
         spawn(1)      message_admins("There can be only one Avatar in the world")
         qdel(src)

      demon=src
      avatarcreated = 1      //Ѕеременную нельз€ у«ƒановиƒь на 0 без Ѕедальвме∆аƒель«ƒва. ≈«ли аваƒар «о«нул, ƒо ново о не «делаƒь.
      if(ticker.mode.name == "cult")      summoned_during_cult=1

      var/matrix/M = matrix()      //”величиƒь «Ѕрайƒ јваƒара. Ёƒо делаеƒ е о мыльным, но ƒуƒ уже выбор - либо карликовый но нормальный, либо нормальный, но мыльный.
      M.Scale(1.25)
      M.Translate(0,5)
      src.transform = M
      ...
      if(!iscultist(src) && summoned_during_cult && mind)
         ...
         src.mind.current.verbs -= /mob/living/proc/cult_innate_comm      //≈му эƒо не нужно

      spawn(1)
         if(!src.giveSpells())      //Ќа ло украдено у ревенанƒа
   ...
   Stat()      //Cƒаƒ-Ѕанелька « временем и  олодом
      ..()
   ...
   Life()
      ...
      for(var/mob/living/M in view(src.loc,3))      //≈ о аура
         if(M)
            if(victimcheck(M))
      ...
      if(health <=200 && enraged==0)      //яро«ƒь
...
/mob/living/simple_animal/avatar/proc/giveSpells()      //Ѕрок, коƒорый выдаЄƒ ему абилки. Ќа ло украдено у –евенанƒа.
   if(src.mind)
...
/obj/effect/proc_holder/spell/targeted/soul_absorb/cast(list/targets)
   for(var/mob/living/carbon/Q in targets)
      spawn(0)
         if(!Q)      return
         if(!Q.ckey && !Q.get_ghost())      //¬орваƒь«€ в  енеƒику, Ѕережраƒь макак и Ѕобедиƒь? Ќеm.
         ...
         demon.adjustBruteLoss(-50)      //ќƒн€ƒь врем€ оƒ оƒ«чЄƒа, Ѕо илиƒь аваƒара и «делаƒь е о менее  олодным.
         demon.hunger=200
...
/obj/effect/proc_holder/spell/aoe_turf/summon_daemons/cast(list/targets)      //Conjure - дерьмо.
   ...
   var/time=300      //¬рем€ «колько они «уще«ƒвуюƒ
   if(demon.enraged)      time=150

   for(var/turf/target_tile in targets)
      if(!target_tile)      return
      for(var/mob/living/M in target_tile.contents)      //Ѕи нуƒь назад ƒе , кƒо р€дом
         if(M!=demon)
            M << "\red You was pushed away by appearing figures!"
            step_away(M,demon)
            M.Weaken(2)
      if(!istype(target_tile,/turf/space) && !(is_blocked_turf(target_tile)))      //≈«ли в €ро«ƒи, ƒо во«емь демонов Ѕо€в€ƒ«€ вокру . ≈«ли неƒ, ƒо чеƒыре, «лева,«Ѕрава, «Ѕереди и «зади.
...
/obj/effect/proc_holder/spell/targeted/suffer/cast(list/targets)
   for(var/mob/living/T in targets)
      ...
      demon.say("COGNOSCE DOLOR")      //≈«ли ебанЄƒ не ƒо о - в«Є равно на кулдаун
...
/obj/effect/proc_holder/spell/targeted/inflict_handler/harpoon
   amt_weakened = 4
   amt_dam_brute = 25      //ЅоЅадание  арЅуном валиƒ « но  и нано«иƒ 25 бруƒа
   ...
   spawn(0)
      for(var/mob/living/jertva in targets)
         ...
         while(get_dist(demon,jertva)>1 && i<=10)
            ++i
            jertva.loc=get_step_towards(jertva,demon)      //„ƒобы через «ƒолы Ѕролеƒала. ≈«ли между ними окажеƒ«€ «ƒена, ƒо эƒо ƒиЅ ƒЄмна€ «ила, ми«ƒика, короч не ба  а фича
            sleep(2)
   ...
   spawn(0)
      ...
      for(var/turf/T in targets)
         ...
         for(var/mob/living/K in T.contents)
            ...
            if(istype(K,/mob/living/silicon))      //Ѕор оƒу ƒуда же
         ...
         for(var/obj/structure/window/W in T.contents)      if(prob(33))      W.spawnfragments()      //–и«к ла алища, Ѕоэƒому ƒуƒ ∆ан« в«е да 1/3


//// Doom


/mob/living/proc/Doom(var/countdown = 60)      //„ерез оƒдельный Ѕрок
   ...
   spawn(1)
      ...
      while(countdown>0)
         ...
         --countdown
         switch(countdown)      //Ѕиблей«кий «ƒи∆ок. ¬озможно не «амое эЅичное или Ѕод од€щее, но Ѕо мне норм.
            ...
            if(-INFINITY to 0)
               for(var/mob/living/K in oview(5, src))      //Ѕрыжок на «лучайную цель р€дом
...
/obj/effect/proc_holder/spell/aoe_turf/inferno/cast(list/targets)
   ...
   for(var/turf/simulated/T in range(demon,7))
      ...
      if(get_dist(T,stone)==7)
         var/obj/effect/forcefield/flames = new /obj/effect/forcefield(T)      //Ќикƒо ниче о не заЅодозриƒ...
         flames.name= "hotspot"
         ...
         flames.luminosity = 5
         walls+=flames
      else      T.color="#383838"      //¬«Є чƒо внуƒри «ƒанеƒ чЄрным
...
/mob/living/simple_animal/avatar/ex_act(severity)      //Ќу ебануƒь одной лимиƒкой, ну ве«ь фан же на уй, ну.
