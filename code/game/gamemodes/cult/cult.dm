...
/datum/game_mode/cult/check_finished()
   if(avatarcreated && !demon && summoning_in_progress)
      ...
      for(var/mob/living/M in world)
         if(istype(M,/mob/living/simple_animal/construct) || istype(M,/mob/living/simple_animal/hostile/faithless))      //ÊîíÇÄðóêÄîâ íåÄ
            M << "\blue \italic Your master was banished from this world, his grip on you...no more. You are free..."
            M.death(1)
         if(iscultist(M))      ticker.mode.remove_cultist(M.mind)      //ÊóëüÄèÇÄîâ íåÄ
      for(var/obj/effect/rune/ru in world)      //Ðóí íåÄ
         qdel(ru)
      for(var/turf/corrupted in world)      //ÊóëüÄîÊîâíà íåÄ
