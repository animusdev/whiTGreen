...
/obj/effect/proc_holder/changeling/proc/can_sting(var/mob/user, var/mob/target)
   ...
   if(req_human && !ishuman(user))
      user << "<span class='warning'>Мы не можем ЗделаДь эДоКо в данной форме!</span>"
      return 0
   var/datum/changeling/c = user.mind.changeling
   if(c.chem_charges<chemical_cost)
      user << "<span class='warning'>Нам нужно КоД&#255; бы [chemical_cost] единиц КимикаДов.</span>"
      return 0
   if(c.absorbedcount<req_dna)
      user << "<span class='warning'>Нам нужно КоД&#255; бы [req_dna] образцов ЗовмеЗДимыК ДНК.</span>"
      return 0
   if(req_stat < user.stat)
      user << "<span class='warning'>Мы иЗДощены!</span>"
      return 0
   if((user.status_flags & FAKEDEATH) && name != "Regenerate")
      user << "<span class='warning'>Мы иЗДощены!</span>"
      return 0
   if(c.geneticdamage > max_genetic_damage)
      user << "<span class='warning'>НаЖ Кеном вЗе еще БереформировываеДЗ&#255;. Нам нужно некоДорое врем&#255; дл&#255; воЗЗДановлени&#255;.</span>"
      return 0
   return 1
