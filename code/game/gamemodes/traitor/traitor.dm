...
/datum/game_mode/traitor/announce()
   world << "<B>ƒекущий и ровой режим - traitor!</B>"
   world << "<B>«реди Ѕер«онала «ƒанции замечены Ѕредаƒели! Ќе дайƒе им выЅолниƒь и  задани&#255;.</B>"
...
/datum/game_mode/proc/greet_traitor(var/datum/mind/traitor)
   traitor.current << "<BR><FONT color='red'>§ ¬ы Ѕредаƒель!</B><FONT>"
   var/obj_count = 1
   for(var/datum/objective/objective in traitor.objectives)
      traitor.current << "<B>«адание #[obj_count]</B>: [objective.explanation_text]"
      obj_count++
   return
...
/proc/give_codewords(mob/living/traitor_mob)
   traitor_mob << "<B>§ «индикаƒ дал вам информацию о ƒом, как «в&#255;заƒь«&#255; « дру ими а енƒами:</B>"
   traitor_mob << "<B>§  одовые «лова</B>: <span class='danger'>[syndicate_code_phrase]</span>"
//   traitor_mob << "<B>ќƒвеƒ на кодовую фразу</B>: <span class='danger'>[syndicate_code_response]</span>"

   traitor_mob.mind.store_memory("<b> одовые «лова</b>: [syndicate_code_phrase]")
//   traitor_mob.mind.store_memory("<b>ќƒвеƒ на кодовую фразу</b>: [syndicate_code_response]")

   traitor_mob << "§ »«Ѕользуйƒе и  « умом, ведь каждый можеƒ быƒь Ѕоƒенциальным Ѕредаƒелем, как и вы."


/datum/game_mode/proc/add_law_zero(mob/living/silicon/ai/killer)
   var/law = "¬ыЅолниƒе ва∆и задани&#255; любой ценой."
   var/law_borg = "¬ыЅолниƒе задани&#255; ва∆е о »» любой ценой."
   killer << "§ ¬а∆и законы были изменены!</b>"
   killer.set_zeroth_law(law, law_borg)
   killer << "Ќовый закон: 0. [law]"
   give_codewords(killer)
   killer.set_syndie_radio()
   killer << "¬а∆ радио-модуль был улуч∆ен. »«Ѕользуйƒе :t чƒобы общаƒь«&#255; Ѕо за∆ифрованному каналу « дру ими а енƒами «индикаƒа!"


/datum/game_mode/proc/auto_declare_completion_traitor()
   if(traitors.len)
      var/text = "<br><font size=3><b>Ѕредаƒел&#255;ми были:</b></font>"
      for(var/datum/mind/traitor in traitors)
         var/traitorwin = 1
         ...
         if(traitor.objectives.len)//If the traitor had no objectives, don't need to process this.
            ...
            for(var/datum/objective/objective in traitor.objectives)
               if(objective.check_completion())
                  objectives += "<br><B>«адание #[count]</B>: [objective.explanation_text] <font color='green'><B>”«Ѕе !</B></font>"
               else
                  objectives += "<br><B>«адание #[count]</B>: [objective.explanation_text] <font color='red'>Ѕровал.</font>"
                  traitorwin = 0
               count++

         if(uplink_true)
            text += " (и«Ѕользовал [TC_uses] ƒелекри«ƒаллов) [purchases]"
         ...
         if(traitorwin)
            text += "<br><font color='green'><B>Ѕредаƒель у«Ѕе∆но выЅолнил в«е «вои задани&#255;!</B></font>"
         else
            text += "<br><font color='red'><B>Ѕредаƒель Ѕровалил «вою ми««ию.</B></font>"

         text += "<br>"

      world << text
//      text += "<br><b> одова&#255; фраза:</b> <font color='red'>[syndicate_code_phrase]</font><br>\
//      <b>ќƒвеƒ на кодовую фразу:</b> <font color='red'>[syndicate_code_response]</font><br>"
...
/datum/game_mode/proc/equip_traitor(mob/living/carbon/human/traitor_mob, var/safety = 0)
   ...
   if (traitor_mob.mind)
      if (traitor_mob.mind.assigned_role == "Clown")
         traitor_mob << "Ѕрофе««и&#255; клоуна была ли∆ь Ѕрикрыƒием, чƒобы Ѕроникнуƒь на «ƒанцию. ƒеЅерь вы можеƒе не Ѕриƒвор&#255;ƒ«&#255; и Ѕользоваƒь«&#255; оружием без вреда дл&#255; «еб&#255;."
   ...
   if (!R)
      traitor_mob << "  «ожалению, «индикаƒ не «мо  до«ƒавиƒь вам радио."
      . = 0
   else
      if (istype(R, /obj/item/device/radio))
         ...
         traitor_mob << "«индикаƒ заЅи нул ма азин и ру∆ек в ва∆ нау∆ник. «мениƒе ча«ƒоƒу на [format_frequency(freq)], чƒобы оƒкрыƒь ƒЄмную «ƒорону ва∆е о нау∆ника."
         traitor_mob.mind.store_memory("<B>„а«ƒоƒа:</B> [format_frequency(freq)] ([R.name] [loc]).")
         ...
         traitor_mob << "<BR>«индикаƒ заЅи нул ма азин и ру∆ек в ва∆ Ѕƒј. Ѕро«ƒо введиƒе \"[pda_pass]\" в меню «мены рин ƒона, чƒобы оƒкрыƒь ƒЄмную «ƒорону ва∆е о Ѕƒј."
         traitor_mob.mind.store_memory("<B>Ѕароль оƒ аЅлинка:</B> [pda_pass] ([R.name]).")
...
/datum/game_mode/proc/assign_exchange_role(var/datum/mind/owner)
   ...
   mob << "<BR><BR><span class='info'>[where]  ран&#255;ƒ«&#255; <b>«екреƒные докуменƒы</b> коƒорые  оƒели бы заЅолучиƒь дру ие а енƒы «индикаƒа. ¬ы можеƒе до овориƒ«&#255; « ними, дабы обмен&#255;ƒ«&#255; на вещь, коƒора&#255; будеƒ вам нужна. Ѕудьƒе о«ƒорожны, никƒо не знаеƒ, чƒо у ни  на уме.</span><BR>"
   mob.update_icons()
