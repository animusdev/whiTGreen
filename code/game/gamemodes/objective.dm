...
datum/objective/assassinate/update_explanation_text()
   ..()
   if(target && target.current)
      explanation_text = "”бейƒе [target.name]."
   else
...
datum/objective/maroon/update_explanation_text()
   if(target && target.current)
      explanation_text = "Ѕозабоƒьƒе«ь о ƒом, чƒобы [target.name] не Ѕокинул «ƒанцию живым."
   else
      explanation_text = "«вободное задание."
...
datum/objective/debrain/update_explanation_text()
   ..()
   if(target && target.current)
      explanation_text = "»звлекиƒе и украдиƒе моз  [target.name]."
   else
      explanation_text = "«вободное задание."
...
datum/objective/protect/update_explanation_text()
   ..()
   if(target && target.current)
      explanation_text = "«ащищайƒе [target.name]."
   else
      explanation_text = "«вободное задание."
...
datum/objective/hijack
   explanation_text = "”леƒиƒе на эвакуационном ∆аƒƒле в одиночку."
   dangerrating = 25
...
datum/objective/block
   explanation_text = "Ќе доЅу«ƒиƒе, чƒобы  оƒ&#255; бы одна о раниче«ка&#255; форма жизни Ѕокинула «ƒанцию на ∆аƒƒле."
   dangerrating = 25
   martyr_compatible = 1
...
datum/objective/escape
   explanation_text = "Ѕокиньƒе «ƒанцию на ∆аƒƒле или «Ѕа«аƒельной ∆люЅке, не будучи Ѕойманным."
   dangerrating = 5
...
datum/objective/escape/escape_with_identity/update_explanation_text()
   if(target && target.current)
      ...
      explanation_text = "Ѕокинуƒь «ƒанцию на ∆аƒƒле или «Ѕа«аƒельной каЅ«уле в обличии [target_real_name]"
      ...
      if(H && H.get_id_name() != target_real_name)
         ...
         explanation_text += ", надев [target.current.gender == "male" ? "е о" : "еЄ"] ID карƒу"
...
datum/objective/martyr
   explanation_text = "”мриƒе «лавной «мерƒью!"
   dangerrating = 1
...
datum/objective/nuclear
   explanation_text = "”ничƒожьƒе «ƒанцию Ѕри Ѕомощь &#255;дерной бомбы."
   martyr_compatible = 1
...
datum/objective/steal/proc/set_target(var/datum/objective_item/item)
   if(item)
      ...
      explanation_text = "”крадиƒе [targetinfo.name]."
...
datum/objective/destroy/update_explanation_text()
   ..()
   if(target && target.current)
      explanation_text = "”ничƒожьƒе эк«Ѕерименƒальный и«ку««ƒвенный инƒеллекƒ [target.name]."
   else
      explanation_text = "«вободное задание."
...
datum/objective/attune/proc/gen_amount_goal(var/lowbound = 4, var/highbound = 6)
   target_amount = rand (lowbound,highbound)
   explanation_text = "Ѕорабоƒиƒе [target_amount] разумов."
   return target_amount
