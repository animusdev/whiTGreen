...
proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, flame_range = 0 ,silent = 0)
	...
	devastation_range=max(0,devastation_range)
	heavy_impact_range=max(0,heavy_impact_range)
	light_impact_range=max(0,light_impact_range)
	flash_range=max(0,flash_range)
	flame_range=max(0,flame_range)
	if(devastation_range+heavy_impact_range+light_impact_range+flash_range+flash_range==0)
		return
	spawn(0)
		var/vertdist=1
		var/epicenter_above=turf_above(epicenter)
		while(epicenter_above)
			explosion_onez(epicenter_above,expl_sqrt(devastation_range**2-vertdist**2),expl_sqrt(heavy_impact_range**2-vertdist**2),expl_sqrt(light_impact_range**2-vertdist**2),expl_sqrt(flash_range**2-vertdist**2),0,ignorecap,expl_sqrt(flame_range**2-vertdist**2),silent)
			epicenter_above=turf_above(epicenter_above)
			vertdist+=1
		vertdist=1
		var/epicenter_below=turf_below(epicenter)
		while(epicenter_below)
			explosion_onez(epicenter_below,expl_sqrt(devastation_range**2-vertdist**2),expl_sqrt(heavy_impact_range**2-vertdist**2),expl_sqrt(light_impact_range**2-vertdist**2),expl_sqrt(flash_range**2-vertdist**2),0,ignorecap,expl_sqrt(flame_range**2-vertdist**2),silent)
			epicenter_below=turf_below(epicenter_below)
			vertdist+=1
...
proc/explosion_onez(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, flame_range = 0 ,silent = 0)
	src = null	//so we don't abort once src is deleted
	epicenter = get_turf(epicenter)
