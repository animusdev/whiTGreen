...
/obj/item/weapon/storage/secure/attack_self(mob/user as mob)
	...
	if ((src.l_set == 0) && (!src.emagged) && (!src.l_setshort))
		dat += text("<p>\n<b>3-DIGIT PASSCODE NOT SET.<br>ENTER NEW PASSCODE.</b>")
	if (src.emagged)
	...
	var/datum/browser/popup = new(user, "briefcase", "")
...
/obj/item/weapon/storage/secure/Topic(href, href_list)
	...
	if (href_list["type"])
		if (href_list["type"] == "E")
			if ((src.l_set == 0) && (length(src.code) == 3) && (!src.l_setshort) && (src.code != "ERROR"))
				src.l_code = src.code
				src.l_set = 1
