...
/obj/item/weapon/picket_sign/attack_self(mob/living/carbon/human/user)
   if( last_wave + 20 < world.time )
      ...
      if(label)
         user.visible_message("<span class='warning'>[user] разма иваеƒ ƒабличкой « надЅи«ью \"[label]\".</span>")
      else
         user.visible_message("<span class='warning'>[user] разма иваеƒ Ѕу«ƒой ƒабличкой.</span>")
