...
/obj/item/weapon/c4/suicide_act(var/mob/user)
   ...
   var/message_say = "јлла  јкбар!"
   if(user.mind)
      if(user.mind.special_role)
         var/role = lowertext(user.mind.special_role)
         if(role == "traitor" || role == "syndicate")
            message_say = "«а «индикаƒ!"
         else if(role == "changeling")
            message_say = "«а ”лей!"
         else if(role == "cultist")
            message_say = "«а „ернобо а!"
         else if(role == "revolutionary" || role == "head revolutionary")
            message_say = "ƒа здрав«ƒвуеƒ революци&#255;!"
...
/obj/item/weapon/c4/afterattack(atom/movable/target, mob/user, flag)
   ...
   if(ismob(target))
      ...
      user.visible_message("<span class='warning'>[user.name] Ѕыƒаеƒ«&#255; ЅрикреЅиƒь взрывчаƒку на [target.name]!</span>")
