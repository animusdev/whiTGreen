...
/datum/paint_info/proc/validate(var/turf/simulated/floor/test)
	...
	switch(ftype)
		if(PAINT_FLOOR) //why is it named plasteel anyway?
			if(test.floor_tile != /obj/item/stack/tile/plasteel)
...
/obj/item/weapon/tile_painter/interact(var/mob/user = usr)
	...
	//var/datum/browser/popup = new(user, "tilepainter","Floor Painter",nref=src)
