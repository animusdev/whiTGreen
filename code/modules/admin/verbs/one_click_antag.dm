...
/datum/admins/proc/one_click_antag()
	...
	var/dat = {"<B>Quick-Create Antagonist</B><br>
		...
		<a href='?src=\ref[src];makeAntag=17'>Make The Thing</a><br>
		"}
...
/datum/admins/proc/makeTheThing()

	var/datum/game_mode/the_thing/temp = new
	if(config.protect_roles_from_antagonist)
		temp.restricted_jobs += temp.protected_jobs
	...
	for(var/mob/living/carbon/human/applicant in player_list)
		if(applicant.client.prefs.be_special & BE_THING)
			if(!applicant.stat)
				if(applicant.mind)
					if (!applicant.mind.special_role)
						if(!jobban_isbanned(applicant, "the thing") && !jobban_isbanned(applicant, "Syndicate"))
							candidates += applicant

	if(candidates.len)
		var/numThing = min(candidates.len, 3)
