...
proc/slur(phrase)
   ...
   for(var/i = 1; i <= lentext(phrase); i++)
      ...
      if(prob(33))
         ...
         if(lowerrustext(letter)=="з")   letter="З"
         if(lowerrustext(letter)=="в")   letter="ф"
         if(lowerrustext(letter)=="б")   letter="Б"
         if(lowerrustext(letter)=="К")   letter="К"
         if(lowerrustext(letter)=="д")   letter="Д"
...
/proc/stutter(phrase)
   ...
   for(var/word in unstuttered_words)
      ...
      if(first_letter == "&")
         first_letter = "&#255;"
      if(upperrustext(first_letter) in list("Б","В","К","Д","Ж","З","К","Л","М","Н","Б","Р","З","Д","Ф","К","Ц","Ч","Ж","Щ"))
         switch(rand(1,3))
            if(1)
