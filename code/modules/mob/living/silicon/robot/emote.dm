/mob/living/silicon/emote(var/act,var/m_type=1,var/message = null)
   ...
   switch(act)//01000001011011000111000001101000011000010110001001100101011101000110100101111010011001010110010000100001 (Seriously please keep it that way.)
      ...
      if("beep")                  //MOB NEEDED
         ...
         if (param)
            message = "<B>[src]</B> «и налиƒ на [param]."
         else
      ...
      if ("buzz")
         ...
         if (param)
            message = "<B>[src]</B> жужжиƒ на [param]."
         else
            message = "<B>[src]</B> жужжиƒ."
         playsound(src.loc, 'sound/machines/buzz-sigh.ogg', 50, 0)
         m_type = 2

      if ("buzz2")
         message = "<B>[src]</B> дважды жужжиƒ."
         playsound(loc, 'sound/machines/buzz-two.ogg', 50, 0)
         m_type = 2

      if ("chime") //You have mail!
         message = "<B>[src]</B> звениƒ."
      ...
      if ("deathgasp")
         message = "<B>[src]</B> вздра иваеƒ и Ѕрекращаеƒ дви аƒь«&#255;,  лаза Ѕо«ƒеЅенно заƒу аюƒ..."
         m_type = 1
      ...
      if ("ping")               //MOB NEEDED
         ...
         if(!M)
            param = null
         if (param)
            message = "<B>[src]</B>  удиƒ на [param]."
         else
            message = "<B>[src]</B>  удиƒ."
         playsound(src.loc, 'sound/machines/ping.ogg', 50, 0)
         m_type = 2

      if ("sad") //When words cannot express...
         message = "<B>[src]</B> Ѕрои рываеƒ  ру«ƒную мелодию."
