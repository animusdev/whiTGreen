...
/mob/living/verb/succumb(var/whispered as null)
	...
	if (InCritical())
		...
		updatehealth()
		if(!whispered)
			src << "<span class='notice'>- ¬ы прекратили свою борьбу за жизнь и отдались смерти. -</span>"
		death()
...
/mob/living/proc/mob_sleep()
	...
	if(sleeping)
		src << "<span class='notice'>- ¬ы уже спите. -</span>"
		return
	else
		if(alert(src, "’отите немного поспать?", "Sleep", "ƒа", "Ќет") == "ƒа")
			sleeping = input(usr," ак долго вы собираетесь отдыхать (в секундах)?","Sleep duration",sleeping ? sleeping : 60) as num
	update_canmove()
...
/mob/living/proc/lay_down()
	...
	resting = !resting
	src << "<span class='notice'>- ¬ы [resting ? "ложитесь" : "поднимаетесь на ноги"]. -</span>"
	update_canmove()
...
/mob/living/stripPanelEquip(obj/item/what, mob/who, where)
	...
	if(what && (what.flags & NODROP))
		src << "<span class='warning'>- ¬ы не можете передать [(what.r_name ? what.r_name : what.name)]! -</span>"// TODO: accusative_case needed
		return
