/mob/living/simple_animal/metroid/verb/Feed()
...
/mob/living/simple_animal/metroid/proc/CanFeedon(var/mob/living/M)
	if(!Adjacent(M))
		return 0
	...
	if(ismetroid(M))
		src << "<i>I can't latch onto another metroid...</i>"
		return 0
	...
	for(var/mob/living/simple_animal/slime/met in view())
		if(met.Victim == M && met != src)
			...
			return 0
	for(var/mob/living/simple_animal/metroid/met in view())
		if(met.Victim == M && met != src)
			src << "<i>The [met.name] is already feeding on this subject...</i>"
			return 0
	return 1

/mob/living/simple_animal/metroid/proc/Feedon(var/mob/living/M)
	...
	icon_state = "[is_adult ? "baby" : "adult"] metroid eat"
	Victim = M
	src.loc = M.loc
	canmove = 0
	anchored = 1
	...
	while(Victim && Victim == M && Victim.stat != DEAD && stat != DEAD)
		...
		if(Adjacent(Victim))
			...
			if(iscarbon(Victim))
				Victim.adjustCloneLoss(rand(9,11))
				Victim.adjustToxLoss(rand(3,4))
				if(Victim.health <= 0)
					Victim.adjustToxLoss(rand(5,7))
				...
				Victim.adjustBruteLoss(is_adult ? rand(10, 17) : rand(6, 15))
				fed_succesfully = 1
...
/mob/living/simple_animal/metroid/proc/Feedstop()
	regenerate_icons()
	if(Victim)
		if(Victim.client)
...
/mob/living/simple_animal/metroid/proc/UpdateFeed(var/mob/M)
	if(Victim)
		if(Victim == M)
			loc = M.loc // simple "attach to head" effect!


/mob/living/simple_animal/metroid/verb/Evolve()
	set category = "Metroid"
	...
	if(!is_adult)
		if(amount_grown >= 10)
			...
			amount_grown = 0
			regenerate_icons()
			name = text("[is_adult ? "adult" : "baby"] metroid ([number])")
		else
			src << "<i>I am not ready to evolve yet...</i>"
	else
		src << "<i>I have already evolved...</i>"

/mob/living/simple_animal/metroid/verb/Reproduce()
	set category = "Metroid"
	...
	if(is_adult)
		if(amount_grown >= 10)
			...
			var/mob/living/simple_animal/metroid/new_slime = pick(babies)
