/mob/living/carbon/alien/humanoid/emote(var/act)
   ...
   switch(act) //Alphabetical please
      if ("deathgasp")
         message = "<span class='name'>[src]</span> издаЄƒ «лабеющий виз , зелЄна&#255; кровь Ѕузыриƒ«&#255; из Ѕа«ƒи..."
         m_type = 2
      ...
      if ("hisss")
         if(!muzzled)
            message = "<span class='name'>[src]</span> ∆иЅиƒ."
            m_type = 2
      ...
      if ("roar")
         if (!muzzled)
            message = "<span class='name'>[src]</span> рычиƒ!"
            m_type = 2
      ...
      if ("scratch")
         if (!src.restrained())
            message = "<span class='name'>[src]</span> че∆еƒ«&#255;."
            m_type = 1
